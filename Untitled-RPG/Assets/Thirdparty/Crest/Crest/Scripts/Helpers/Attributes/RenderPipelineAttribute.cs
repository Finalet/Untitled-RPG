        ...
        internal override void Decorate(Rect position, SerializedProperty property, GUIContent label, DecoratedDrawer drawer)
        {
            switch (pipeline)
            {
                case RenderPipeline.Legacy:
                    if (!RenderPipelineHelper.IsLegacy) DecoratedDrawer.HideInInspector = true;
                    break;
                case RenderPipeline.HighDefinition:
                    if (!RenderPipelineHelper.IsHighDefinition) DecoratedDrawer.HideInInspector = true;
                    break;
                case RenderPipeline.Universal:
                    if (!RenderPipelineHelper.IsUniversal) DecoratedDrawer.HideInInspector = true;
                    break;
                default: break;
            }
        }
