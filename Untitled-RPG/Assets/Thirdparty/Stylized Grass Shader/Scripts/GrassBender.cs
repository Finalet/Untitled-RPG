        ...
        private void OnDrawGizmos()
        {
            if (meshFilter)
            {
                if (benderType == GrassBenderBase.BenderType.Mesh && meshFilter.sharedMesh)
                {
                    ...
                    Gizmos.DrawWireMesh(meshFilter.sharedMesh, 0, meshFilter.transform.position, meshFilter.transform.rotation, meshFilter.transform.lossyScale * scaleMultiplier);
                    Gizmos.color = new Color(0.5f, 0.75f, 1, 0.3f);
                    Gizmos.DrawMesh(meshFilter.sharedMesh, 0, meshFilter.transform.position, meshFilter.transform.rotation, meshFilter.transform.lossyScale * scaleMultiplier);
                }
            }
        }
    }
