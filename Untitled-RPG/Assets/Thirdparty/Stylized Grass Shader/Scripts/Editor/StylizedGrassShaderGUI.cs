...
namespace StylizedGrass
{
    public class StylizedGrassShaderGUI : ShaderGUI
    {
        ...
        private MaterialProperty specular;
        ...
        public void FindProperties(MaterialProperty[] props)
        {
            ...
            hasColorMap = Shader.GetGlobalVector("_ColorMapUV").w == 1f;
            ...
            specular = FindProperty("_SpecularAmount", props);
        ...
        private void DrawColor()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (m_ColorOptionsFoldout)
                {
                    ...
                    if (!hasColorMap) EditorGUILayout.HelpBox("No color map is currently active", MessageType.None);
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (m_ShadingOptionsFoldout)
                {
                    ...
                    if (advancedLighting.floatValue == 1f)
                    {
                        ...
                        if (environmentReflections.floatValue == 0f && RenderSettings.ambientMode != AmbientMode.Skybox)
                        {
                            EditorGUILayout.HelpBox("Environment lighting source must be set to \"Skybox\" for reflections to have an effect", MessageType.Warning);
                    ...
                    DrawSlider(specular, "Controls how strongly the sun is reflected into the material. Similar to material wetness");
        ...
        private void DrawBending()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                GUILayout.Space(-5f);
                m_BendOptionsFoldout = EditorGUILayout.BeginFoldoutHeaderGroup(m_BendOptionsFoldout, new GUIContent("Bending"));
                if (m_BendOptionsFoldout)
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        bendMode.floatValue = (float)GUILayout.Toolbar((int)bendMode.floatValue,
                            new GUIContent[] { new GUIContent("Per-vertex"), new GUIContent("Whole object") }
                            );
                    }
                    DrawSlider(bendPushStrength, "The amount of pushing the material should receive by Grass Benders");
                    DrawSlider(bendFlattenStrength, "A multiplier for how much the material is flattened by bending");
        ...
        private void DrawWind()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (m_WindOptionsFoldout)
                {
                    ...
                    materialEditor.ShaderProperty(windDirection, windDirection.displayName);
