...
public class HuntersCage : Skill
{
    [Header("Skill Vars")]
    ...
    protected override void CastingAnim() {
        animator.CrossFade("Attacks.Hunter.Hunters Cage", 0.25f);
        StartCoroutine(SpawnArrowIE());
        PlaySound(castingSound, 0.15f, 0.6f * characteristics.castingSpeed.x);
    ...
    IEnumerator SpawnArrowIE () {
        newArrow = Instantiate(arrowPrefab, WeaponsController.instance.BowObj.transform).GetComponent<ArrowCage>();
        while (newArrow != null) {
